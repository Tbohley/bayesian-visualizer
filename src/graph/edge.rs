...
pub fn on_node_drag(
    ...
    for (link_entity, link_component) in graph_links.iter_mut() {
        ...
        {
            let (new_transform, new_mesh) =
                link_transform_helper(&link_component, &transforms, &mut meshes).unwrap();
...
pub fn spawn_finished_link(
    ...
    let length = (from_pos.distance(to_pos) - 2.0 * RANDOM_NODE_RAD).max(0.0);
    commands
        .spawn((
            ...
            Transform {
                translation: from_pos.lerp(to_pos, 0.5),
